...
module.exports = {
  ...
  async execute(message, args, client) {    
    ...
    if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("**vous n'avez pas la permission d'effectuer cette action !**");
            if (!message.guild.me.hasPermission("KICK_MEMBERS")) return message.channel.send("**Je n'ai pas l'autorisation de kicker les utilisateurs ! | [KICK_MEMBERS]**");
            if (!args[0]) return message.channel.send('You must mention a `User` to **kick**.')

            let banMember = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.guild.members.cache.find(r => r.user.username.toLowerCase() === args[0].toLocaleLowerCase()) || message.guild.members.cache.find(ro => ro.displayName.toLowerCase() === args[0].toLocaleLowerCase());
                 if (banMember === message.member) return message.channel.send("Tu ne peux pas te botter.")

            var reason = args.slice(1).join(" ");

            if (!banMember.kickable) return message.channel.send("Il semble que l'utilisateur spécifié ne puisse pas être expulsé.")
            try {
